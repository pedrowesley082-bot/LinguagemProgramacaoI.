# Linguagem de Programação I

**Nome:** Pedro Wesley de Sousa de Andrade  
**Curso e Turma:** Licenciatura em Computação – 2025.1  

# O que espero aprender na disciplina

Espero adquirir conhecimentos sólidos sobre **lógica de programação**, utilizando a linguagem **Pascalzim**, que considero uma excelente porta de entrada para linguagens de programação mais avançadas.
Como aluno que possui uma base quase inexistente em programação, acredito que iniciar meus estudos por uma linguagem simples e bem estruturada, como o Pascal, será fundamental para compreender os conceitos básicos da programação e construir uma base sólida para aprendizados futuros.

# Meu interesse em programação

Desde muito jovem sempre tive grande interesse por jogos digitais e curiosidade em entender como eles eram criados. Com o tempo, descobri que os jogos envolvem diversos conceitos da programação, como lógica, estrutura e criatividade.
Esse interesse me motivou a aprender programação, principalmente a lógica por trás do funcionamento dos sistemas, para que futuramente eu possa desenvolver meus próprios projetos — quem sabe até criar um jogo desenvolvido por mim. A possibilidade de criar algo do zero por meio da programação é algo que considero realmente fascinante.
